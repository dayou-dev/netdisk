...
<div class="btn-toolbar list-toolbar">
    <form class="form-inline" id="formval" action="<?php echo $this->_var['webadmin']; ?>?ctl=admin" method="post">
    <button class="btn btn-default" onClick="checkAll(this);" type="button"><i class="fa fa-square-o"></i> 全选</button>
    <button class="btn btn-default" type="button" onClick="location.href='?ctl=admin&act=add'"><i class="fa fa-plus"></i> 添加</button>
...
<th>登录IP</th><th><a href="javascript:sortBy('is_effect','1','PaymentOther','index')" title="按照类型     升序排列 ">状态     </a></th>
<th><a href="javascript:sortBy('login_time','1','PaymentOther','index')" title="按照付款单金额  升序排列 ">最后登录时间</a></th>
...
<td align="center"><a href="<?php echo $this->_var['webadmin']; ?>/?ctl=admin&act=edit&id=<?php echo $this->_var['item']['id']; ?>" target="" title="<?php echo $this->_var['item']['title']; ?>"><?php echo $this->_var['item']['adm_name']; ?></a></td><td align="center"><?php echo $this->_var['item']['login_ip']; ?></td>
...
?></td>
<td align="center">
          <a href="<?php echo $this->_var['webadmin']; ?>/?ctl=admin&act=edit&id=<?php echo $this->_var['item']['id']; ?>" title="编辑"><i class="fa fa-pencil"></i></a>
...
function delsel(paramobj)
{
	...
	dialog({title:"提示",content:'确定要删除选定项目吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=admin&act=foreverdelete&id="+id, 
				data: "ajax=1",
				dataType: "json",
...
function del(id){
	dialog({title:"提示",content:'确定要删除吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=admin&act=delete&id="+id, 
				data: "ajax=1",
				dataType: "json",
				success: function(obj){
					if(obj.status==1)
