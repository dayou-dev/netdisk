...
<div class="theme-blue"><?php echo $this->fetch('top.html'); ?></div>
    ...
    <div class="content">
        <div class="header">
            ...
            <li class="active">会员<?php if ($this->_var['id']): ?>编辑<?php else: ?>添加<?php endif; ?></li>
        </ul>
...
<ul class="nav nav-tabs">
  <li class="active"><a href="#home" data-toggle="tab">会员<?php if ($this->_var['id']): ?>编辑<?php else: ?>添加<?php endif; ?></a></li>
</ul>
...
<div class="row">
  <div class="col-md-4" style="width: 80%;">
    ...
    <div id="myTabContent" class="tab-content">
      ...
      <form id="formval" action="<?php echo $this->_var['webadmin']; ?>?ctl=user&act=<?php if ($this->_var['vo']['id']): ?>update<?php else: ?>insert<?php endif; ?>" method="post">
        <div class="form-group">
        <label>登录名  [<font color="#FF0000">*</font>]</label>
        
        <?php if ($this->_var['id']): ?><?php echo $this->_var['vo']['user_name']; ?><?php else: ?><input type="text" name="user_name" id="user_name" value="" class="form-control" intchk="*"><?php endif; ?>
        </div>
        ...
        <div class="form-group">
        <label>手机</label>
        <input type="text" name="mobile" id="mobile" value="<?php echo $this->_var['vo']['mobile']; ?>" class="form-control" >
        </div>
        
        <div class="form-group">
        <label>Email  [<font color="#FF0000">*</font>]</label>
        <input type="text" name="email" id="email" value="<?php echo $this->_var['vo']['email']; ?>" class="form-control" intchk="*">
        </div>
        ...
        <div class="form-group">
			...
			<?php $_from = $this->_var['userGroup']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'item');if (count($_from)):
    foreach ($_from AS $this->_var['item']):
?>
            <option value="<?php echo $this->_var['item']['id']; ?>" style="background-color:#F6F6F6;" <?php if ($this->_var['item']['id'] == $this->_var['vo']['group_id']): ?> selected="selected"<?php endif; ?>><?php echo $this->_var['item']['title']; ?></option>
