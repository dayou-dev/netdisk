...
<div class="btn-toolbar list-toolbar">
    <form class="form-inline" id="formval" action="<?php echo $this->_var['webadmin']; ?>?ctl=article" method="post">
    <button class="btn btn-default" onClick="checkAll(this);" type="button"><i class="fa fa-square-o"></i> 全选</button>
    <button class="btn btn-default" type="button" onClick="location.href='?ctl=article&act=add'"><i class="fa fa-plus"></i> 添加</button>
...
<th><a href="javascript:sortBy('cate_id','1','PaymentOther','index')" title="按照付款单金额  升序排列 ">分类  </a></th><th><a href="#">来源</a></th><th><a href="javascript:sortBy('is_effect','1','PaymentOther','index')" title="按照类型     升序排列 ">状态     </a></th><th><a href="#">原链接</a></th><th><a href="javascript:sortBy('create_time','1','PaymentOther','index')" title="按照创建时间     升序排列 ">创建时间     </a></th><th><a href="javascript:sortBy('update_time','1','PaymentOther','index')" title="按照收款时间  升序排列 ">操作  </a></th>
</tr>
</thead>
<tbody>
...
<td align="center"><a href="/<?php echo $this->_var['webadmin']; ?>?ctl=article&act=edit&id=<?php echo $this->_var['item']['id']; ?>" target="" title="<?php echo $this->_var['item']['title']; ?>"><?php echo $this->_var['item']['title']; ?></a></td><td align="center"><?php echo $this->_var['item']['cate_name']; ?></td>
<td align="center">
<?php echo $this->_var['item']['source']; ?>
</td><td align="center"><?php if ($this->_var['item']['is_effect']): ?>有效<?php else: ?><font color="#999">无效</font><?php endif; ?></td>
<td align="center"><?php if ($this->_var['item']['rel_url']): ?><a href="<?php echo $this->_var['item']['rel_url']; ?>" target="_blank">查看</a><?php else: ?>---<?php endif; ?></td>
<td align="center"><?php 
$k = array (
  'name' => 'to_date',
...
?></td>
<td align="center">
          <a href="/<?php echo $this->_var['webadmin']; ?>?ctl=article&act=edit&id=<?php echo $this->_var['item']['id']; ?>" title="编辑"><i class="fa fa-pencil"></i></a>
...
function delsel(paramobj)
{
	...
	dialog({title:"提示",content:'确定要删除选定项目吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=article&act=foreverdelete&id="+id, 
				data: "ajax=1",
				dataType: "json",
...
function del(id){
	dialog({title:"提示",content:'确定要删除吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=article&act=delete&id="+id, 
				data: "ajax=1",
				dataType: "json",
				success: function(obj){
					if(obj.status==1)
