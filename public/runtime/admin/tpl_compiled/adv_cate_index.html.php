...
<div class="btn-toolbar list-toolbar">
    <form class="form-inline" id="formval" action="<?php echo $this->_var['webadmin']; ?>?ctl=advCate" method="post">
    <button class="btn btn-default" onClick="checkAll(this);" type="button"><i class="fa fa-square-o"></i> 全选</button>
    <button class="btn btn-default" type="button" onClick="location.href='?ctl=advCate&act=add'"><i class="fa fa-plus"></i> 添加</button>
...
<td align="center">
          <a href="<?php echo $this->_var['webadmin']; ?>/?ctl=advCate&act=edit&id=<?php echo $this->_var['item']['id']; ?>" title="编辑"><i class="fa fa-pencil"></i></a>
...
function delsel(paramobj)
{
	...
	dialog({title:"提示",content:'确定要删除选定项目吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=advCate&act=foreverdelete&id="+id, 
				data: "ajax=1",
				dataType: "json",
...
function del(id){
	dialog({title:"提示",content:'确定要删除吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=advCate&act=delete&id="+id, 
				data: "ajax=1",
				dataType: "json",
				success: function(obj){
					if(obj.status==1)
