...
<div class="btn-toolbar list-toolbar">
    <form class="form-inline" id="formval" action="<?php echo $this->_var['webadmin']; ?>?ctl=company" method="post">
    <button class="btn btn-default" onClick="checkAll(this);" type="button"><i class="fa fa-square-o"></i> 全选</button>
    <button class="btn btn-default" type="button" onClick="location.href='?ctl=company&act=add'"><i class="fa fa-plus"></i> 添加</button>
    ...
    <select name="cate_id"  class="form-control select_i">
			...
			<?php $_from = $this->_var['company_cate']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'item');if (count($_from)):
...
<th><a href="javascript:sortBy('sort','1','PaymentOther','index')" title="按照类型  升序排列 ">排序</a></th>
...
<td align="center"><a href="/<?php echo $this->_var['webadmin']; ?>?ctl=company&act=edit&id=<?php echo $this->_var['item']['id']; ?>" target="" title="<?php echo $this->_var['item']['title']; ?>"><?php echo $this->_var['item']['title']; ?></a></td>
...
?></td>
<td align="center">
          <a href="/<?php echo $this->_var['webadmin']; ?>?ctl=company&act=edit&id=<?php echo $this->_var['item']['id']; ?>" title="编辑"><i class="fa fa-pencil"></i></a>
...
function del(id){
	dialog({title:"提示",content:'确定要删除吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=company&act=delete&id="+id, 
				data: "ajax=1",
				dataType: "json",
				success: function(obj){
					if(obj.status==1)
