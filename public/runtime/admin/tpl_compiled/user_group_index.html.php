...
<div class="btn-toolbar list-toolbar">
    <form class="form-inline" id="formval" action="<?php echo $this->_var['webadmin']; ?>?ctl=user_group" method="post">
    <button class="btn btn-default" onClick="checkAll(this);" type="button"><i class="fa fa-square-o"></i> 全选</button>
    <button class="btn btn-default" type="button" onClick="location.href='?ctl=user_group&act=add'"><i class="fa fa-plus"></i> 添加</button>
    <button class="btn btn-default" type="button" onClick="delsel()"><i class="fa fa-trash-o"></i> 删除</button>
          <div class="search_txt">  

    
        <input name="wordkey" type="text" id="wordkey" class="input-xlarge form-control" placeholder="名称" value="<?php echo $_REQUEST['wordkey'];?>" style="margin-right:3px;">
...
<th><a href="javascript:sortBy('sort','1','PaymentOther','index')" title="按照付款单金额  升序排列 ">排序</a></th>
...
<td align="center"><a href="<?php echo $this->_var['webadmin']; ?>/?ctl=user_group&act=edit&id=<?php echo $this->_var['item']['id']; ?>" target="" title="<?php echo $this->_var['item']['title']; ?>"><?php echo $this->_var['item']['title']; ?></a></td>
<td align="center"><?php echo $this->_var['item']['sort']; ?></td>
<td align="center">
          <a href="<?php echo $this->_var['webadmin']; ?>/?ctl=user_group&act=edit&id=<?php echo $this->_var['item']['id']; ?>" title="编辑"><i class="fa fa-pencil"></i></a>
...
function delsel(paramobj)
{
	...
	dialog({title:"提示",content:'确定要删除选定项目吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=user_group&act=foreverdelete&id="+id, 
				data: "ajax=1",
				dataType: "json",
...
function del(id){
	dialog({title:"提示",content:'确定要删除吗?',okValue:"确定",ok:function(){
		...
		$.ajax({ 
				url: "/<?php echo $this->_var['webadmin']; ?>?ctl=user_group&act=delete&id="+id, 
				data: "ajax=1",
				dataType: "json",
				success: function(obj){
					if(obj.status==1)
