...
class Baofoo_payment implements payment {
	...
	public function get_payment_code($payment_notice_id) {
        ...
        $_Merchant_url =  get_domain().APP_ROOT.'/baofoo_callback.php?act=response';
        //$_Return_url =$_GET['linkUrl'] ? rawurlencode($_GET['linkUrl']) : get_domain().APP_ROOT.'/baofoo_callback.php?act=notify';
		$_Return_url = get_domain().APP_ROOT.'/baofoo_callback.php?act=notify';
		if($_GET['linkUrl']){
			setcookie("paylinkUrl",$_GET['linkUrl']);
		}else{
			setcookie("paylinkUrl","");
		}
        ...
        $def_url = '<form style="text-align:center;" action="'.get_domain().APP_ROOT."/baofoo_post.php".'"  style="margin:0px;padding:0px" method="POST" >';
        ...
        $def_url .= "</form>";
        $def_url.="<br /><div style='text-align:center' class='red'>".$GLOBALS['lang']['PAY_TOTAL_PRICE'].":".format_price($_OrderMoney)."</div><div style=\" text-align:center; line-height:30px; font-size:12px;margin-top:10px;color:#ff0000\">信用卡充值所产生的转账费用，按转账金额0.5%，由第三方平台收取。</div>";
        return $def_url;
    }
        ...
        if ($Md5Sign != $WaitSign) {
			...
			if($rs)
			{
				...
				if($rs)
				{
					...
					if($_COOKIE['paylinkUrl']){
						header("location:".$_COOKIE['paylinkUrl']);
					}else{
						if($order_info['type']==0){
							//app_redirect(url("index","payment#done",array("id"=>$payment_notice['order_id']))); //支付成功
							app_redirect("/payment/done/".$payment_notice['order_id'].".html");
						}
						else{
							//app_redirect(url("index","payment#incharge_done",array("id"=>$payment_notice['order_id']))); //支付成功
							app_redirect("/payment/incharge_done/".$payment_notice['order_id'].".html");
						}
					}
				...
				else 
				{
					...
					if($_COOKIE['paylinkUrl']){
						...
						{				
							...
							app_redirect("/payment/pay/".$payment_notice['id'].".html");
    ...
    public function get_display_code() {
		...
		if($payment_item)
		{
			...
			$html = "<style type='text/css'>.baofoo_types{float:left; display:block;text-align:left; margin: 10px 0px;margin-bottom: 0px;}";
	        $html .=".bk_type_0{background:url(".get_domain().APP_ROOT."/system/payment/Baofoo/baofoo.jpg) no-repeat right center; font-size:0px; width:150px; height:10px;}.baofoo_types input{margin-top: 15px;}"; 
	        $html .="</style>";
	       ...
	       foreach ($payment_cfg['baofoo_gateway'] AS $key=>$val)
	        {
	            ...
	            $html  .= "<div class='baofoo_types bk_type_".$key."'><input type='radio' name='payment' value='".$payment_item['id']."' rel='".$key."' onclick=\"set_bank('".$key."')\" /> <dt title='".$this->payment_lang['baofoo_gateway_'.$key]."'></dt> </div>";
	        }
