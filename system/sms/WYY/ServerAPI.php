...
class ServerAPI{
     ...
     * @return $请求返回结果(array)
     */
    public function postDataFsockopen($url,$data,$AppKey='',$AppSecret=''){
     ...
     * @return $请求返回结果(array)
     */
    public function postDataFsockopen_1($url,$data,$AppKey='',$AppSecret=''){
		if($AppKey) $this->AppKey = $AppKey;
		if($AppSecret) $this->AppSecret = $AppSecret;
	    $this->checkSumBuilder();       //发送请求前需先生成checkSum
        $postdata = '';
     ...
     */
    public function sendSMSTemplate($templateid,$mobiles=array(),$params='',$AppKey,$AppSecret){
        ...
        if($this->RequestType=='curl'){
            ...
            $result = $this->postDataFsockopen($url,$data,$AppKey,$AppSecret);
        }
     ...
     * @return $result      [返回array数组对象]
     */
    public function sendSMSTemplate_1($templateid,$mobile,$codeLen=4,$deviceId='',$AppKey,$AppSecret){
        ...
        if($this->RequestType=='curl'){
            ...
            $result = $this->postDataFsockopen_1($url,$data,$AppKey,$AppSecret);
        }
