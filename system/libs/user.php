...
define("ACCOUNT_NO_VERIFY_ERROR",3); //帐户未激活
	...
	{		
		...
		//开始数据验证
		$user_data['user_name']=addslashes(htmlspecialchars($user_data['user_name']));
		$user_data['user_pwd']=addslashes(htmlspecialchars($user_data['user_pwd']));
		$user_data['mobile']=addslashes(htmlspecialchars($user_data['mobile']));
	 ...
	 */
	function modify_account($data,$user_id,$log_msg='',$adminedit=0)
	{
		...
		{		
			...
			$log_info['ymoney'] = floatval($user_info['money']);
			$log_info['user_id'] = $user_id;
			$log_info['adminedit'] = $adminedit;
	...
	{
		$user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where (user_name='".$user_name_or_email."' or mobile = '".$user_name_or_email."') and is_delete = 0");
	...
	{
		$user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where (mobile = '".trim($user_name_or_email)."') and is_delete = 0");
		if(!$user_data)
		{			
			$result['status'] = 0;
		...
		else
		{
			...
			else
			{
				...
				es_session::set("user_info",$user_data);
				$GLOBALS['user_info'] = $user_data;
				$GLOBALS['db']->query("update ".DB_PREFIX."user set login_ip = '".get_client_ip()."',login_time= ".get_gmtime()." where id =".$user_data['id']);				
				$s_api_user_info = es_session::get("api_user_info");
				return $result;
			}
		}
